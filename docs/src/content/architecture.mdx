...
Two co-deployed Workers:

- **App Worker** at `template.<subdomain>.workers.dev` — serves the SPA via Workers Assets and the API via Hono. Bound to D1.
- **Docs Worker** at `template-docs.<subdomain>.workers.dev` — this React + Mantine + MDX site, served as static assets from a Worker.
